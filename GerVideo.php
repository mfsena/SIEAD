...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frmVideos")) {
  $insertSQL = sprintf("INSERT INTO tbcadvideo (DataVideo, HorarioIniVideo, HorarioFimVideo, NomeVideo, TemaVideo, UserProf) VALUES (%s, %s, %s, %s, %s, %s)",
					...
					<div id="middlepic">
						<div id="contenttxt">
								<table border="1" cellpadding="0" cellspacing="0" style="text-align: left; height: 40px;">
											<th style="vertical-align: central; size:24px; text-align: center; background-color: rgb(40,88,133); color: white;"><strong>Gerenciar Videos</strong></th>
								</table>
								<h2>&nbsp;</h2>
								<div id="formWrap"></div>
					<table width="512px">
						...
						<th>Tema</th>
								<?php do { ?>
									<tr>
									<td style="width:80px;border-right: solid thin"><?php echo $row_rstVideos['Data']; ?></td>
									<td style="width:60px;border-right: solid thin"><?php echo $row_rstVideos['HorarioIniVideo']; ?></td>
									<td style="width:60px; border-right: solid thin"><?php echo $row_rstVideos['HorarioFimVideo']; ?></td>
									<td style="width:300px; border-right: solid thin"><?php echo $row_rstVideos['NomeVideo']; ?></td>
									</tr>
								<?php } while ($row_rstVideos = mysql_fetch_assoc($rstVideos)); ?>
					</table>
					</b>
					
							<tr>
									<td bgcolor="#FFFFFF"><?php if ($pageNum_rstVideos > 0) { // Show if not first page ?>
													<a href="<?php printf("%s?pageNum_rstVideos=%d%s", $currentPage, 0, $queryString_rstVideos); ?>"><img src="/SIEAD/images/First.gif"></a>
										<?php } // Show if not first page ?></td>
									<td><?php if ($pageNum_rstVideos > 0) { // Show if not first page ?>
													<a href="<?php printf("%s?pageNum_rstVideos=%d%s", $currentPage, max(0, $pageNum_rstVideos - 1), $queryString_rstVideos); ?>"><img src="/SIEAD/images/Previous.gif"></a>
										<?php } // Show if not first page ?></td>
									<td><?php if ($pageNum_rstVideos < $totalPages_rstVideos) { // Show if not last page ?>
													<a href="<?php printf("%s?pageNum_rstVideos=%d%s", $currentPage, min($totalPages_rstVideos, $pageNum_rstVideos + 1), $queryString_rstVideos); ?>"><img src="/SIEAD/images/Next.gif"></a>
										<?php } // Show if not last page ?></td>
									<td><?php if ($pageNum_rstVideos < $totalPages_rstVideos) { // Show if not last page ?>
													<a href="<?php printf("%s?pageNum_rstVideos=%d%s", $currentPage, $totalPages_rstVideos, $queryString_rstVideos); ?>"><img src="/SIEAD/images/Last.gif"></a>
										<?php } // Show if not last page ?></td>
							</tr>
